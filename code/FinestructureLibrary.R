...
{
    ## Convert a binary matrix (with individuals as rows and populations as columns) into a list of populations, where each population is a vector of individual IDs
...
popTableAsList <-function(popids){
    ## Convert a population ID table (with individual IDs in the row names and population IDs in the first column) into a list of populations, where each population is a vector of individual IDs
    pops <- unique(popids[,1])
    poplist <- lapply(pops,function(x) rownames(popids)[popids[,1]==x])
    names(poplist) <- pops
    poplist
}
...
dendAsPopList<-function(dend){
    ## converts a dendrogram into a list of populations, where the population names are the labels of the dendrogram
    mapstatelist<-lapply(labels(dend),getIndivsFromSummary)
    names(mapstatelist)<-SafeNameMoreSummary(mapstatelist) # a nicer summary of the populations
    mapstatelist
}
...
NameNoSummary<-function(x){ ## Write a semicolon separated list of the individuals in a population
    paste0(x,collapse=";")
}
...
extend <- function(alphabet) function(i) {
    ## Extends an alphabet to as many letters as needed, by using the base10toA function to convert numbers to letters
...
SafeNameMoreSummary = function(x){
    ## Make names of the form A_label, B_label, etc. to avoid problems with long names and duplicates
    ## if you have more than 26 populations, it will start with AA, AB, etc. and so on
  res = sapply(x,NameMoreSummary)
  fullletters=extend(LETTERS)
  res =paste(fullletters(1:length(res)),res,sep="_")
  res
}

summarizeNames<-function(l,omitbelow=0,fracbelow=0,addcount=FALSE,counteach=FALSE){
    ...
    alltab<-sapply(l,function(x){
        retnames<-sort(unique(unlist(l)))
        if(length(retnames)==0) return(rep(0,sum(x==y)))
        sapply(retnames,function(y) sum(na.omit(x)==y))
    })
    alltab2<-alltab/rowSums(alltab)
    temppoplabels<-sapply(1:length(l),function(i){
        x=l[[i]]
        x<-gsub("[0-9,-]","",x)
        tmp<-order(alltab2[alltab2[,i]>0,i],decreasing=TRUE) #[1:sum(alltab2[,i]>0)]
        ttab<-table(x)[tmp]
        tcumsum<-c(1,1-cumsum(ttab)/sum(ttab))[1:length(ttab)]
        tkeep<-ttab>omitbelow & tcumsum>fracbelow
        tkeep[1]<-TRUE
        ttab<-ttab[tkeep]
        if(counteach) names(ttab)<-paste0(names(ttab),ttab)
...
NameLessSummary<-function(x){
    ## Takes a list of names and collapses them to a single name, with the individuals separated by semicolons
  paste(x,collapse=";")
}
...
extractTree <- function(txml,idtable=NULL,labmode="mcmc",oneminus=TRUE,hidecertain=FALSE){
    ## extracts the tree from the MAP/Tree file xml
    tmptree<-txml$doc$children$outputFile[[length(txml$doc$children$outputFile)]]
    t<-read.tree(text=xmlValue(tmptree))
    t$node.label<-as.vector(sapply(t$node.label,simplelabels,labmode=labmode,oneminus=oneminus))
    if(hidecertain) t$node.label[which(t$node.label=="1")]<-""
    class(t)<-"phylo"
    if(!any(is.null(idtable))){
        t$tip.label=idtable[t$tip.label,1]
    }
    return(t)
}

popAsList <- function(s,idtable=NULL) {
## convert bracketed population string into population list
    s<-gsub(")","",s)
    s<-strsplit(s,c("(",")"),fixed=TRUE)[[1]]
    s<-s[sapply(s,nchar)>0]
    ret=sapply(s,strsplit,",",fixed=TRUE)
    if(!any(is.null(idtable))){
        ret=sapply(ret,function(x)idtable[x,1])
        names(ret)=sapply(ret, function(x)paste(x,collapse=",")[[1]])
    }
    ret
}

poplistAsIdTable<-function(cutstatelist,ids){
    ## Takes a list of cut states and a list of ids, and returns a table of the population of each id in each cut state
    ## ids are needed for sorting the individuals into the original order.
    tlist=lapply(names(cutstatelist),function(n){
        cbind(cutstatelist[[n]],n)
    })
    tmat=do.call(rbind,tlist)
    rownames(tmat)=tmat[,1]
    res=tmat[ids,,drop=FALSE]
...
dendrogramMergeSmall<-function(tdend,minsize=10,summary=NameSummary,retainmembers=TRUE){
    ## Merges small clusters in a dendrogram, by replacing them with a single leaf with the summary of the labels of the individuals within it. 
    ## If retainmembers is FALSE, then the members attribute of the new leaf is set to 1 (instead of the number of members in the cluster) and the height is set to 0 (instead of the height of the original cluster). 
    ## This is useful for plotting, as it makes small clusters look like leaves, but it means that you can't use the members attribute to get the number of individuals in each cluster.
    tms<-function(x){
        if(is.leaf(x)) return(x)       
        if(attr(x[[1]],"members")<minsize || attr(x[[2]],"members")<minsize) {
            ret<-numeric()
            if(!retainmembers){
                attr(ret,"members")<-1 #attr(x,"members")
            }
            #attr(ret,"midpoint")<-attr(x,"midpoint")
            attr(ret,"height")<-0 # attr(x,"height")
            attr(ret,"label")<-summary(labels(x))
            attr(ret,"leaf")<-TRUE
            class(ret)<-"dendrogram"
            return(ret)
        }
    ...
    ret<-tms(tdend)
    ret<-midcache.dendrogram(ret)
    ret
}

idsAssign<-function(ids,poplist){
    ## Takes the list of ids, and a populuation list, and gives
    ## all individuals in a population the name of that population as their
    ## population field, as well as including them in the analysis. All unlisted individuals are excluded
    labelmapClusters<-list()
    for(i in 1:length(poplist)) for(x in poplist[[i]]) {
        labelmapClusters[[x]] <-names(poplist)[i]
    }
    
    idsclusters<-as.data.frame(matrix(NA,nrow=dim(ids)[1],ncol=3))
    idsclusters[,1]=ids[,1]
    if(dim(ids)[2]>=2) {idsclusters[,2]=ids[,2]
                    }else idsclusters[,2]="ignore"
    idsclusters[,3]=0
    
    for(i in 1:dim(idsclusters)[1]){
        tw<-labelmapClusters[[idsclusters[i,1]]]
        if(!is.null(tw)) {
            idsclusters[i,2]<-tw
            idsclusters[i,3]<-1
        }else{
...
expandDendrogramLabels<-function(tdend){ 
    ## takes each label and applies a name expansion to it. Then applies a contraction to simplify the names once again
    dendrapply(tdend,function(x){
        if(is.leaf(x)) attr(x,"label")<-NameMoreSummary(NameExpand(attr(x,"label")))
        x
    })
}

uniqueCutHeights<-function(tdend){ 
    ## Useful for getting "all K" for cutting of a tree
...
rgbDistCols<-function(mydata, colorder=c(1,2,3,4),colmax=0.8, alphamin=0.5){ ## Makes a colour for each row of the data by embedding it in 4 dimensions, rgbalpha
    ## colorder allows relabelling of colour directions and changing polarity (if -ve)
    ## colmax in the maximum a colour can be (for preventing white)
    ## alphamin is the minimum the alpha can be (for preventing colours from being too faded)
    require("MASS")
...
#    fit <- isoMDS(d, k=4) # k is the number of dim
    ...
    rgb(mymap(emb[,abs(colorder[1])],colorder[1])*colmax,
...
readIndexedState<-function(x){
## converts a state in the form of c(1,1,1,2,3,3,3,4) to standard form
  tstate<-matrix(0,ncol=max(x),nrow=length(x))
  for(i in 1:length(x)){
    tstate[i,x[i]]<-1
  }
  tstate
...
pcaMclust<-function (tdatatest, Gmax =NULL)
{
    if(is.null(Gmax)) stop("Must provide a maximum number of clusters, Gmax")
    require("mclust")
    .Mclust <- mclust.options(emModelNames = c("EII", "VII"),
        hcModelNames = c("EII", "VII"), warn = FALSE)
    tresmcBIC <- mclustBIC(tdatatest, G = 2:Gmax, control = emControl(eps = 1e-31,
        tol = c(1e-04, 1e-05), itmax = 1e+05))
    tresmc <- mclustModel(tdatatest, tresmcBIC)
    while (is.null(tresmc$z)) {
        Gmax <- Gmax - 1
        tresmcBIC <- mclustBIC(tdatatest, G = 1:Gmax)
        tresmc <- mclustModel(tdatatest, tresmcBIC)
        if (Gmax == 1)
            return(NULL)
    }
    tresmcmap <- apply(tresmc$z, 1, function(x) {
        which(x == max(x))
    })
    .Mclust <- mclust.options()
    list(state=readIndexedState(tresmcmap),par=tresmc$parameters)
}
...
drawScale<-function(x,mycols,scalemar=c(6,2,1,2),scalenum=6,
                    scalesignif=2,trans=identity,labeltrans=identity,vert=FALSE) {
    colscale<-range(x)
    par(mar=scalemar)
    colindex<-(matrix(seq(min(colscale),max(colscale),
                      length.out=100),ncol=1,nrow=100)) # colour scale
    if(vert){
        image(1,1:100,trans(colindex),xaxt="n",yaxt="n",xlab="",
          ylab="",col=mycols,zlim=range(trans(colindex)))
    }else{image(1:100,1,trans(colindex),xaxt="n",yaxt="n",xlab="",
          ylab="",col=mycols,zlim=range(trans(colindex)))}
    scalelocs <- min(colindex) + (max(colindex)-min(colindex)) *seq(0,1,length.out=scalenum)
    scalephysicalpos <- seq(1,100,length.out=scalenum)
    axis(1+as.numeric(vert),at=scalephysicalpos,
         labels=signif(labeltrans(scalelocs),scalesignif),las=2,cex.axis=1)
}
