...
## setwd("simple_europe_example") # set the working directory to the example directory
source("../code/FinestructureLibrary.R") # read in the R functions, which also calls the needed packages

dir.create("results") # make a directory for the results to go in
...
chunkfile<-paste0(dir,"/testcombined.chunkcounts.out") ## chromopainter chunkcounts file
mcmcfile<-paste0(dir,"/testcombined.mcmc.xml") ## finestructure mcmc file
treefile<-paste0(dir,"/testcombined.tree.xml") ## finestructure tree file
treefile_wh<-paste0(dir,"/testcombined.tree.wh.xml") ## finestructure tree

## Set ip the idtable to rename data
idtable=read.table("rawdata/panel.ids")[,1,drop=FALSE]
idtable[,2]=paste0("IND",1:dim(idtable)[1])
rownames(idtable)=idtable[,2]

read_matrix<-function(file,idtable,row.names=1,header=T,skip=0){
    dataraw<-as.matrix(read.table(chunkfile,row.names=row.names,header=header,skip=skip)) # read in the pairwise coincidence 
    rownames(dataraw)<-colnames(dataraw)<-idtable[rownames(dataraw),1]
    dataraw
}

###### READ IN THE CHUNKCOUNT FILE
dataraw<-read_matrix(chunkfile,idtable,skip=1) # read in the pairwise coincidence 
...
## plot(mcmcdata$Number, mcmcdata$Posterior,cex=2) ## Check the MCMC trace (number of iterations vs posterior probability)
...
ttree_wh<-extractTree(treexml_wh,idtable) ## extract the tree into ape's phylo format
...
## Cut the tree at the height of 100 log-likelihood units
cutat=100
tdend2<-cutdend(tdend_wh,cutat)
print(paste("Using",length(labels(tdend2)),"clusters")) # number of clusters in the cut tree
## Same things for our cut state
cutstatelist<-dendAsPopList(tdend2)
cutnamesplot <- names(cutstatelist)

newpoplist=NameExpand(labels(tdend2))

##### This is for plotting only

## And the normal version
treexml<-xmlTreeParse(treefile) ## read the tree as xml format
ttree<-extractTree(treexml,idtable) ## extract the tree into ape's phylo format
...
pdf(file="results/CuttingDendrograms.pdf",height=10,width=10)
par(mfrow=c(3,1))
...
mapstateunsorted<-popAsList(extractValue(treexml,"Pop"),idtable) # map state as a finestructure clustering

popdend<-makemydend(tdend,mapstateunsorted)
mapstatelist <- dendAsPopList(popdend)
popnamesplot<-names(mapstatelist)

popidtable<-poplistAsIdTable(cutstatelist,idtable[,1])
write.table(popidtable,file="testcombined.pop.ids",quote=FALSE,row.names=FALSE,col.names=FALSE)
...
cutdendclear<-cutdend(tdend_wh,cutat,"NameMoreSummary")
...
pdf(file="results/Example2CoancestryCut.pdf",height=12,width=12)
...
pdf(file="results/Example2CoancestryPop.pdf",height=12,width=12)
...
pdf(file="results/Example2CoancestryRaw.pdf",height=12,width=12)
...
tmpmat[tmpmat>tmatmax]<-tmatmax # 
pdf(file="results/Example2CoancestryPopAveFull.pdf",height=12,width=12)
